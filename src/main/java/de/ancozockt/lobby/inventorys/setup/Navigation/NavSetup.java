...
public class NavSetup {
    ...
    public static void openSetUp(Player p){
        ...
        for(NavItem item : Main.getInstance().getInventoryManager().getNavConfig().getItems().values()){
            inv.setItem(item.getPosition(), item.getItem());
        }

        for(NavGameItem item : Main.getInstance().getInventoryManager().getNavConfig().getGames().values()){
            inv.setItem(item.getPosition(), item.getItem());
        }
